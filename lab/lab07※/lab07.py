...
def inc_subseqs(s):
    ...
    def subseq_helper(s, prev):
        if not s:
            ...
            return insert_into_all(s[0], inc_subseqs(s[1:])) + inc_subseqs(s[1:])
        else:
            a = s[0]
            b = [i for i in inc_subseqs(s[1:]) if not i or i[0] >= a]
            return insert_into_all(a, b) + inc_subseqs(s[1:])
    return subseq_helper(s, (s + [0, 0])[1])
...
def trade(first, second):
    ...
    while m <= len(first) - 1 and n <= len(second) - 1 and not equal_prefix():
