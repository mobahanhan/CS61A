...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        for ao in player.deck.cards:
            ao.attack += other_card.attack
            ao.defense += other_card.defense
        t = opponent.deck.cards[:]
        for yin in t:
            if yin.attack == other_card.attack or yin.defense == other_card.defense:
                opponent.deck.cards.remove(yin)
