...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        if isinstance(self.operator, LambdaExpr) or isinstance(self.operator, CallExpr):
            return self.operator.eval(env).apply([b.eval(env) for b in self.operands])
        return env[self.operator.__str__()].apply([b.eval(env) for b in self.operands])
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        "*** YOUR CODE HERE ***"
        cmw_env = self.parent.copy()
        insert_mono = {}
        for (para, arg) in zip(self.parameters, arguments):
            insert_mono[para] = arg
        cmw_env.update(insert_mono)
        return self.body.eval(cmw_env)
