...
def read(s): #何谓read指令
    ...
    >>> read('  ')  # returns None
    """
    src = Buffer(tokenize(s)) #对列表化了且处理过的read物执行Buffer类(source)，并把self叫作src
    if src.current() is not None:
        return read_expr(src) #如果还有，就read_expr一下
...
def tokenize(s):
    ...
    ['lambda', 'f', ':', 'f', '(', 0, ',', 4.2, ')']
    """
    src = Buffer(s) #s作为source
    tokens = []
    while True:
        token = next_token(src) #token是处理后的字符
        if token is None:
            return tokens #没有了就输出 
        tokens.append(token) #还有就往里加
