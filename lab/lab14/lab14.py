...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            _ = list(helper_align(skeleton_idx + 1, code_idx + 1))
            #
            possibilities.append((skel_char + _[0], 0 + _[1]))
        # Insert
        edits = "+[" + code_char + "]" 
        __ = list(helper_align(skeleton_idx, code_idx + 1))
        possibilities.append((edits+ __[0], 1 + __[1]))
        # Delete
        edits = "-[" + skel_char + "]"
        ___ = list(helper_align(skeleton_idx + 1, code_idx))
        possibilities.append((edits+ ___[0], 1 + ___[1]))
...
def num_splits(s, d):
    ...
    def nimasile(s, d):
        ...
        for i in range(len(s)):
            copy = s[:]
            fu = copy.pop(i)
            sum += (nimasile(copy, d + fu) + nimasile(copy, d - fu))
        return sum
    jiecheng = 1
    for i in range(1, len(s) + 1):
        jiecheng *= i
    if d == 0:
        return nimasile(s[1:], d + s[0]) * len(s) // jiecheng 
    return nimasile(s, d) * 2 // jiecheng
